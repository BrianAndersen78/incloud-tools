...
if (!class_exists('InCloudAdminPageFramework_Registry', false)):
    abstract class InCloudAdminPageFramework_Registry_Base {
    ...
    final class InCloudAdminPageFramework_Registry extends InCloudAdminPageFramework_Registry_Base {
        ...
        static public function setUp($sFilePath = __FILE__) {
            ...
            self::$sAutoLoaderPath = isset(self::$aClassFiles['InCloudAdminPageFramework_RegisterClasses']) ? self::$aClassFiles['InCloudAdminPageFramework_RegisterClasses'] : '';
            self::$bIsMinifiedVersion = class_exists('InCloudAdminPageFramework_MinifiedVersionHeader', false);
            self::$bIsDevelopmentVersion = isset(self::$aClassFiles['InCloudAdminPageFramework_InclusionClassFilesHeader']);
        }
    ...
    if (!class_exists('InCloudAdminPageFramework_Bootstrap', false)):
        final class InCloudAdminPageFramework_Bootstrap {
            static private $_bLoaded = false;
            public function __construct($sLibraryPath) {
                if (!$this->_isLoadable()) {
                    return;
                }
                InCloudAdminPageFramework_Registry::setUp($sLibraryPath);
                if (InCloudAdminPageFramework_Registry::$bIsMinifiedVersion) {
                    return;
                }
                $this->_include();
            }
            ...
            private function _include() {
                include (InCloudAdminPageFramework_Registry::$sAutoLoaderPath);
                new InCloudAdminPageFramework_RegisterClasses('', array('exclude_class_names' => array('InCloudAdminPageFramework_MinifiedVersionHeader', 'InCloudAdminPageFramework_BeautifiedVersionHeader',),), InCloudAdminPageFramework_Registry::$aClassFiles);
                self::$_bXDebug = isset(self::$_bXDebug) ? self::$_bXDebug : extension_loaded('xdebug');
                if (self::$_bXDebug) {
                    new InCloudAdminPageFramework_Utility;
                    new InCloudAdminPageFramework_WPUtility;
                }
            }
            static private $_bXDebug;
        }
        new InCloudAdminPageFramework_Bootstrap(__FILE__);
    endif;
